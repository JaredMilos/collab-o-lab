...
(function () {
    ...
    var getWhiteKeyWidth = function (number_of_white_keys) {
        return Math.floor(((settings.width - 10) - number_of_white_keys) / number_of_white_keys);
    };
    ...
    var lightenUp = function lightenUp (el) {
        //This has been disabled so that the keys do not light up when chat typing.
        //Has been replaced with a function in the index.ejs file
    ...
    var styleBlackKey = function (key) {
        var white_key_width = getWhiteKeyWidth(getTotalWhiteKeys()),
            black_key_width = Math.floor(white_key_width / 2)-2;
            
        key.el.style.backgroundColor = settings.blackKeyColour;
        key.el.style.border = '1px solid ' + settings.borderColour;
        key.el.style.position = 'absolute';
        key.el.style.left = (Math.floor((white_key_width + 1) * (key.noteNumber + 1) - ((black_key_width / 2)-3))) + 'px';
...
//Somewhere in this function is where we're going to get it to label the keys for musical typing
